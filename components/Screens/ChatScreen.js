...
import { getThisUser, getThisAccountName } from "../../helpers/account";
...
const ChatScreen = () => {
    ...
    const saveToFirebase = (message) => {
        ...
        firebase.database().ref('messages/'+message[0]._id).set(messageToFirebase);    
    }

    return (
        <GiftedChat
            messages={messages}
            user={{_id:getThisUser(), name:accountName}}
            onSend={(message) => {saveToFirebase(message)} }
            showUserAvatar
            scrollToBottom
            />
    );
